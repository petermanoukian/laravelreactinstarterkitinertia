...
import TaggFilter from '@/components/Superadmin/Tagg/TaggFilter';
import TaggTableHeader from '@/components/Superadmin/Tagg/TaggTableHeader'; 
import TaggTableRow from '@/components/Superadmin/Tagg/TaggTableRow'; 
...
import { Button } from '@/Components/ui/button'
 ...
 type Tagg = {
    ...
    export default function ViewTagg({
    taggs,
    filters = {},
    }: {
    taggs: PaginatedTaggs;
    filters?: Filters;
    }) {
    ...
    const applyFilters = useCallback((overrideFilters = {}) => {
        ...
        router.get(route('superadmin.taggs.index'), newFilters, {
    ...
    const sortBy = (field: string) => {
      ...
      const handleDelete = async (id: number) => {
        try 
        {
          const url = route('superadmin.tagg.destroy', { id });
      ...
      const handleDeleteSelected = async () => 
      {
          ...
          try {
              const response = await fetch(route('superadmin.taggs.destroyAll'), {
                  method: 'POST',
                  headers: {
                      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '',
                      'Accept': 'application/json',
  ...
  return (
        ...
        <div className="p-4">
            <p>
                <Link href="/superadmin/tagg/add" prefetch
