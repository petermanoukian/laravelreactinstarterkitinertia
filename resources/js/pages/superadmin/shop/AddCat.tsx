...
const AddCat = () => {
    ...
    useEffect(() => {
        if (datax.name) {
            const timeout = setTimeout(() => {
                axios.post('/superadmin/cats/check-cat-superadmin', { name: datax.name })
                    .then((res) => setCatExists(res.data.exists))
                    .catch(() => setCatExists(false));
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const newErrors: typeof localErrors = {};
            ...
            // Basic text fields
            if (!datax.name.trim()) {
                newErrors.name = 'Categoy Name is required.';
            }
       ...
       if (!catExists) 
        {
            post('/superadmin/cat/store');
        }  

    };

    const checkCat = async (name: string) => {
        if (!name) return;

        try {
            const response = await axios.post(route('superadmin.cats.checkCat'), { name });
    ...
    return (
        ...
        <>
            ...
            <AppLayout>
                ...
                <p>
                    <Link href="/superadmin/cats/view" prefetch 
                ...
                autoComplete="off">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onBlur={(e) => checkCat(e.target.value)}
                            className="w-full border p-2"
                        />
                        {catExists && <div className="text-red-500 text-sm">Category already exists.</div>}
